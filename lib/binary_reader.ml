...
functor
  ...
  struct
    ...
    let bytes l =
      let unsafe_sub s o =
        let dest = Bytes.make l '\000' in
        let () = Bytes.unsafe_blit (Bytes.unsafe_of_string s) o dest 0 l in
    ...
    Functorial with type 'a t = 'a F.t)
...
functor
  ...
  struct
    ...
    module F : Functorial with type 'a t := 'a A.t = Functorial (A)
    include F
...
functor
  ...
  struct
    ...
    include MU
    include MU.Infix
    module A : Applicative with type 'a t := 'a M.t = Applicative (M)
    include A
