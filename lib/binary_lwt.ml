module Reader : sig
  ...
  let read l =
    ...
    in
      Lwt.map
        (fun () -> (s', 0), s'')
        (Lwt_io.read_into_exactly ic (Bytes.unsafe_of_string s') 0 l)
  ;;

  let map f m = fun t s -> Lwt.map (fun (x, s') -> f x, s') (m t s)
  let pure x = fun _ s -> Lwt.return (x, s)

  let apply f x =
    fun t s ->
    let open Lwt in
    f t s >>= fun (f', s') -> Lwt.map (fun (x', s'') -> f' x', s'') (x t s')
  ;;

  let bind k m = fun t s -> Lwt.bind (k t s) (fun (r, s') -> m r t s')

  let run ?(default_size = 1024) ?(max_size = 4096) (t : 'a t) ic =
    let s = String.make default_size '\000' in
