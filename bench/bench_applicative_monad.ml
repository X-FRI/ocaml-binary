...
let monadic =
  ...
  int32le >>= fun a -> int32be >>= fun _ -> int32le >>= fun b -> return (a, b)
;;

let main () =
  let src = "\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c" in
  let a =
    Bench.Test.create ~name:"Applicative" (fun () ->
      ignore (Binary_string.Reader.run applicative src))
  and m =
    Bench.Test.create ~name:"Monadic" (fun () ->
      ignore (Binary_string.Reader.run monadic src))
  in
